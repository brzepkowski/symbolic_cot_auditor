...
step(5, inference, rule_calc_frames_305).
step(6, inference, rule_calc_frames_250).

step(7, inference, rule_calc_remaining_pages).
step(8, inference, rule_calc_average_pages_frames).

step(9, inference, rule_sum_all_frames).
step(10, conclusion, total_frames(7040)).


% Rules
rule(rule_calc_frames_305,
     [pages_with_frames(10,305)],
     frames_from_pages(305,3050)).

rule(rule_calc_frames_250,
     [pages_with_frames(7,250)],
     frames_from_pages(250,1750)).

rule(rule_calc_remaining_pages,
     [total_pages(25),
      pages_with_frames(10,_),
      pages_with_frames(7,_)],
     remaining_pages(8)).

rule(rule_calc_average_pages_frames,
     [remaining_pages(8),
      average_frames_per_page(280)],
     frames_from_average_pages(2240)).

rule(rule_sum_all_frames,
     [frames_from_pages(305,3050),
      frames_from_pages(250,1750),
      frames_from_average_pages(2240)],
     total_frames(7040)).
